...
@Component({
  selector: 'app-join-clinic-request',
  ...
  template: `
    <div class="mx-auto" style="max-width: 760px">
      <a
        ...
        {{ isOnboarding ? 'Wróć do wyboru miejsca pracy' : 'Wróć do klinik' }}
      </a>

      <section class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-4 p-lg-5 text-center">
          <span
            class="d-inline-flex align-items-center justify-content-center rounded-circle bg-warning-subtle text-warning-emphasis fs-2 mb-4"
          ...
          <div class="card bg-body-tertiary border-0 rounded-4 text-start my-4">
            <div class="card-body p-4 d-flex gap-3">
              <span
                class="d-flex align-items-center justify-content-center bg-primary-subtle text-primary rounded-3 flex-shrink-0"
          ...
          <div class="alert alert-light border rounded-3 text-start d-flex gap-3">
            ...
            <div>
              O zmianie statusu poinformujemy Cię w aplikacji. Powiadomienie e-mail może być dodatkowym kanałem.
            </div>
          </div>
          ...
          <div class="alert alert-warning border-0 rounded-3 text-start d-flex gap-3">
            ...
            <div>
              <strong>Nie masz jeszcze dostępu do tej kliniki.</strong>
              Nie pokazujemy jej grafiku ani danych pacjentów, dopóki administrator nie zaakceptuje zgłoszenia.
            </div>
          </div>

          <div class="d-flex flex-column flex-sm-row justify-content-center gap-2 mt-4">
            <a [routerLink]="joinAnotherLink" class="btn btn-primary">Wyślij zgłoszenie do innej kliniki</a>
            @if (isOnboarding) {
...
export class JoinRequestComponent {
  ...
  readonly requestId = this.route.snapshot.paramMap.get('requestId') ?? 'pending';
  ...
  private readonly clinicId = Number(this.route.snapshot.queryParamMap.get('clinicId'));
  readonly clinicName = this.clinicNames[this.clinicId] ?? 'Wybrana placówka';

  readonly backLink = this.isOnboarding ? '/onboarding/vet/workplace' : '/clinics/browse';
  readonly joinAnotherLink = this.isOnboarding ? '/onboarding/vet/join-clinic-clinic' : '/clinics/join-clinic';
